...
//open modal by setting the display to the transaction conponent from
  ...
  if(data.length < 2){
    setComponentToShow(<DeleteBasicBankComponent data={data} xoutHandler={xoutHandler} deleteEverything={deleteEverything} />)
  }else{
    ...
    <div className='bottomsection'>
        ...
        <div id='displayField'>
         ...
         {data.map((acc, index) => {
          ...
          if(acc.accounttype){
      return <AccountCard key={index} depositHandler={startDeposit} withDrawHandler={startWithdrawal} transferHandler={StartTransfer} accountType={acc.accounttype} accountNumber={acc.accountnumber} accountBalance={acc.accountbalance} transferFrom={transferFrom} />
        }     
                            })} 
